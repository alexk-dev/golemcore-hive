...
export function GolemDetailsPanel({
  ...
  return (
    <div className="panel p-5">
      ...
      <div className="mt-4">
        <p className="text-xs font-semibold text-muted-foreground">Roles ({golem.roleSlugs.length})</p>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-border/60 pt-4">
        ...
        {golem.pauseReason ? <span className="text-sm text-sky-900">Paused: {golem.pauseReason}</span> : null}
        {golem.revokeReason ? <span className="text-sm text-rose-900">Revoked: {golem.revokeReason}</span> : null}
      </div>
    </div>
  );
}
