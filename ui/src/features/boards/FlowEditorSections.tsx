    ...
    <div className="grid gap-3">
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          type="button"
          onClick={onAddColumn}
          className="border border-border bg-panel px-3 py-1.5 text-sm font-semibold text-foreground"
        >
          Add column
        </button>
      </div>
      {columns.map((column, index) => (
        <div key={column.id} className="grid gap-2 border border-border bg-muted/70 p-3">
          <div className="grid gap-2 md:grid-cols-[1fr_1fr_100px_90px_auto]">
            ...
            <button
              ...
              className="border border-rose-700 bg-rose-900/40 px-3 py-2 text-sm font-semibold text-rose-300"
            >
              Remove
            </button>
          </div>
    ...
    <div className="grid gap-3">
      ...
      {transitions.map((transition, index) => (
        <div
          ...
          className="grid gap-2 border border-border bg-muted/70 p-3 md:grid-cols-[1fr_1fr_auto]"
        >
          <select
    ...
    <div className="grid gap-3">
      ...
      {signalMappings.map((mapping, index) => (
        <div
          ...
          className="grid gap-2 border border-border bg-muted/70 p-3 md:grid-cols-[1fr_1fr_1fr_auto]"
        >
          <input
...
export function FlowPreviewPanel({
  ...
  if (previewError) {
    return <p className="text-sm text-rose-300">{previewError}</p>;
  }
  if (!preview) {
    return null;
  }
  ...
  return (
    <div className="border border-amber-700 bg-amber-950/40 p-4">
      <p className="text-sm font-semibold text-amber-300">Remap required</p>
      <p className="mt-1 text-sm text-amber-300">
        Cards in removed columns need a new target.
      </p>
      <div className="mt-3 grid gap-2">
        {preview.removedColumnIds.map((columnId) => (
          <label key={columnId} className="grid gap-1.5 md:grid-cols-[1fr_220px] md:items-center">
            <span className="text-sm text-amber-300">
              {columnId} · {preview.affectedCardCounts[columnId] || 0} cards
            </span>
            <select
