...
package me.golemcore.hive.selfevolving.adapter.out.persistence;
...
public class JsonSelfEvolvingProjectionAdapter implements SelfEvolvingReadPort, SelfEvolvingWritePort {
    ...
    @Override
    public void applyEvent(SelfEvolvingProjectionEvent event) {
        switch (event.eventType()) {
        case "selfevolving.run.upserted":
            applyRunProjection(
                    event.golemId(),
                    event.runId(),
                    objectMapper.convertValue(event.payload(), SelfEvolvingRunProjection.class),
                    event.createdAt());
            break;
        case "selfevolving.candidate.upserted":
            applyCandidateProjection(
                    event.golemId(),
                    objectMapper.convertValue(event.payload(), SelfEvolvingCandidateProjection.class),
                    event.createdAt());
            break;
        case "selfevolving.campaign.upserted":
            applyCampaignProjection(
                    event.golemId(),
                    objectMapper.convertValue(event.payload(), SelfEvolvingCampaignProjection.class),
                    event.createdAt());
            break;
        case "selfevolving.lineage.upserted":
            applyLineageProjection(
                    event.golemId(),
                    objectMapper.convertValue(event.payload(), SelfEvolvingLineageNode.class),
                    event.createdAt());
            break;
        case "selfevolving.artifact.upserted":
            applyArtifactCatalogProjection(
                    event.golemId(),
                    objectMapper.convertValue(event.payload(), SelfEvolvingArtifactCatalogProjection.class),
                    event.createdAt());
            break;
        case "selfevolving.artifact.normalized-revision.upserted":
            applyArtifactNormalizedRevisionProjection(
                    event.golemId(),
                    objectMapper.convertValue(event.payload(), SelfEvolvingArtifactNormalizedRevisionProjection.class),
                    event.createdAt());
            break;
        case "selfevolving.artifact.lineage.upserted":
            applyArtifactLineageProjection(
                    event.golemId(),
                    objectMapper.convertValue(event.payload(), SelfEvolvingArtifactLineageProjection.class),
                    event.createdAt());
            break;
        case "selfevolving.artifact.diff.upserted":
            applyArtifactDiffPayload(
                    event.golemId(),
                    objectMapper.convertValue(event.payload(), Map.class),
                    event.createdAt());
            break;
        case "selfevolving.artifact.evidence.upserted":
            applyArtifactEvidencePayload(
                    event.golemId(),
                    objectMapper.convertValue(event.payload(), Map.class),
                    event.createdAt());
            break;
        case "selfevolving.tactic.upserted":
            applyTacticProjection(
                    event.golemId(),
                    objectMapper.convertValue(event.payload(), SelfEvolvingTacticProjection.class),
                    event.createdAt());
            break;
        case "selfevolving.tactic.search-status.upserted":
            applyTacticSearchStatusProjection(
                    event.golemId(),
                    objectMapper.convertValue(event.payload(), SelfEvolvingTacticSearchStatusProjection.class),
                    event.createdAt());
            break;
        default:
            throw new IllegalArgumentException("Unsupported self-evolving eventType: " + event.eventType());
        }
    }

    public void applyRunProjection(
            String golemId,
            String runId,
            SelfEvolvingRunProjection projection,
            Instant createdAt) {
        projection.setGolemId(firstNonBlank(projection.getGolemId(), golemId));
        projection.setId(firstNonBlank(projection.getId(), runId));
        projection.setUpdatedAt(resolveUpdatedAt(createdAt));
        saveRun(projection);
        updateRunProjection(projection);
    }

    public void applyCandidateProjection(String golemId, SelfEvolvingCandidateProjection projection,
            Instant createdAt) {
        projection.setGolemId(firstNonBlank(projection.getGolemId(), golemId));
        projection.setUpdatedAt(resolveUpdatedAt(createdAt));
    ...
    public void applyCampaignProjection(String golemId, SelfEvolvingCampaignProjection projection, Instant createdAt) {
        projection.setGolemId(firstNonBlank(projection.getGolemId(), golemId));
        projection.setUpdatedAt(resolveUpdatedAt(createdAt));
        saveCampaign(projection);
    }

    public void applyLineageProjection(String golemId, SelfEvolvingLineageNode projection, Instant createdAt) {
        projection.setGolemId(firstNonBlank(projection.getGolemId(), golemId));
        projection.setUpdatedAt(resolveUpdatedAt(createdAt));
        saveLineage(projection);
    }

    public void applyArtifactCatalogProjection(
            String golemId,
    ...
    public void applyArtifactNormalizedRevisionProjection(
            String golemId,
    ...
    public void applyArtifactLineageProjection(
            String golemId,
            SelfEvolvingArtifactLineageProjection projection,
            Instant createdAt) {
        projection.setUpdatedAt(resolveUpdatedAt(createdAt));
        saveArtifactLineage(golemId, projection);
    }

    public void applyArtifactDiffPayload(String golemId, Map<String, Object> payload, Instant createdAt) {
        saveArtifactPayload(SELF_EVOLVING_ARTIFACT_DIFFS_DIR, golemId, payload, createdAt);
    }

    public void applyArtifactEvidencePayload(String golemId, Map<String, Object> payload, Instant createdAt) {
        saveArtifactPayload(SELF_EVOLVING_ARTIFACT_EVIDENCE_DIR, golemId, payload, createdAt);
    }

    public void applyTacticProjection(String golemId, SelfEvolvingTacticProjection projection, Instant createdAt) {
        projection.setGolemId(firstNonBlank(projection.getGolemId(), golemId));
        projection.setUpdatedAt(resolveUpdatedAt(createdAt));
        saveTactic(projection);
    }

    public void applyTacticSearchStatusProjection(
            String golemId,
            SelfEvolvingTacticSearchStatusProjection projection,
            Instant createdAt) {
        projection.setGolemId(firstNonBlank(projection.getGolemId(), golemId));
        projection.setUpdatedAt(resolveUpdatedAt(createdAt));
    ...
    private Instant resolveUpdatedAt(Instant createdAt) {
