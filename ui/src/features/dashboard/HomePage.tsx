...
import { listAuditEvents, type AuditEvent } from '../../lib/api/auditApi';
import { listGolems } from '../../lib/api/golemsApi';
import { listObjectives, type ObjectiveDetail } from '../../lib/api/objectivesApi';
import { getOrganization, updateOrganization, type OrganizationDetail } from '../../lib/api/organizationApi';
  ...
  return (
    <div className="grid gap-5">
      ...
      <MetricsSection
        ...
        teamsCount={teams.length}
        activeObjectives={activeObjectives}
        onlineGolems={onlineGolems}
        totalGolems={golems.length}
        pendingApprovalsCount={pendingApprovals.length}
      />
      <div className="grid gap-5 lg:grid-cols-2">
        <ServicesPreview services={services} />
        <ObjectivesPreview objectives={objectives} teamNameById={teamNameById} />
      </div>
      <div className="grid gap-5 lg:grid-cols-2">
        <TeamsPreview teams={teams} />
        <AuditPreview recentAudit={recentAudit} />
      </div>
    </div>
  );
}
    ...
    <section className="panel px-5 py-5">
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.4fr)_320px]">
        ...
        <div className="grid gap-3">
          <QuickLinkCard title="Objectives" value={activeObjectives} detail={`${objectivesCount} total`} to="/objectives" />
          <QuickLinkCard title="Teams" value={teamsCount} detail={`${golemsCount} registered golems`} to="/teams" />
          <QuickLinkCard title="Services" value={servicesCount} detail={`${totalCards} cards in queues`} to="/services" />
        </div>
      </div>
    </section>
  );
...
function MetricsSection({
  ...
  return (
    <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
      <MetricCard label="Services" value={servicesCount} to="/services" />
      <MetricCard label="Teams" value={teamsCount} to="/teams" />
      <MetricCard label="Objectives" value={activeObjectives} to="/objectives" />
      <MetricCard label="Golems online" value={`${onlineGolems}/${totalGolems}`} to="/fleet" />
      <MetricCard label="Pending approvals" value={pendingApprovalsCount} to="/approvals" />
    </section>
  );
}

function ServicesPreview({ services }: { services: ServiceSummary[] }) {
  return (
    ...
    <section className="panel p-5">
      ...
      <div className="mt-3 grid gap-2">
        {objectives.length ? objectives.slice(0, 5).map((objective) => (
          <Link key={objective.id} to="/objectives" className="border border-border/70 bg-muted/70 p-3 transition hover:bg-muted">
            <div className="flex items-center justify-between gap-3">
...
function TeamsPreview({ teams }: { teams: TeamDetail[] }) {
  return (
    <section className="panel p-5">
      <div className="flex items-center justify-between gap-3">
      ...
      <div className="mt-3 grid gap-2">
        {teams.length ? teams.slice(0, 5).map((team) => (
          <Link key={team.id} to="/teams" className="border border-border/70 bg-muted/70 p-3 transition hover:bg-muted">
            <div className="flex items-center justify-between gap-3">
...
function AuditPreview({ recentAudit }: { recentAudit: AuditEvent[] }) {
  return (
    <section className="panel p-5">
      <div className="flex items-center justify-between gap-3">
