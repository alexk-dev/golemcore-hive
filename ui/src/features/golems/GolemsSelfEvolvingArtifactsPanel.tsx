...
import { compareSelfEvolvingArtifacts, searchSelfEvolvingArtifacts } from '../../lib/api/selfEvolvingApi';
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)]">
        <div className="grid gap-2">
          {searchQuery.isLoading ? (
            <p className="text-sm text-muted-foreground">Searching mirrored artifacts...</p>
          ) : (searchQuery.data ?? []).length === 0 ? (
            <p className="text-sm text-muted-foreground">No mirrored artifacts match the current filters.</p>
          ) : (
            (searchQuery.data ?? []).map((artifact) => (
              <article key={`${artifact.golemId}:${artifact.artifactStreamId}`} className="border border-border/70 bg-white/80 p-3">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <button
                    type="button"
                    onClick={() => setLeftSelection(artifact)}
                  ...
                  <button
                    type="button"
                    onClick={() => setRightSelection(artifact)}
                    className="border border-border bg-white px-3 py-1 text-xs font-semibold text-foreground"
                  >
                    Set right
                  </button>
