...
export function TransitionSuggestionBanner({ signal }: TransitionSuggestionBannerProps) {
  ...
  return (
    <section className="border border-amber-700 bg-amber-950/40 px-4 py-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <span className="text-sm font-semibold text-amber-950">{signal.signalType}</span>
          <span className="ml-2 text-sm text-amber-300">{signal.summary}</span>
        </div>
      ...
      {signal.resolutionSummary ? <p className="mt-1 text-xs text-amber-300">{signal.resolutionSummary}</p> : null}
    </section>
  );
}
